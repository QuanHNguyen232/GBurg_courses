...
public class SimpleSATSolver extends SATSolver
{
	...
	public boolean makeQuery() 
	{
		...
		for (int i=0; i<iterations && !unsatClauseIndices.isEmpty(); i++) {
			...
			ArrayList<Integer> newUnsatClauseIndices = getUnsatClausesIndices(allClauses);
			// unflip the assignment if more clauses are unsatisfied (unless small probability)
			if (newUnsatClauseIndices.size() > unsatClauseIndices.size() && random.nextDouble() >= probRandomFlip) {
				assignment[var] = !assignment[var];
				// TODO: missing line
			}
			else {
	...
	private ArrayList<Integer> getUnsatClausesIndices(ArrayList<int[]> allClauses){
		...
		for (int i=0; i<allClauses.size(); i++) {
			...
			for (int j=0; !satisfied && j<clause.length; j++) {
				...
				satisfied = (assignment[varNum] == literal > 0);
			}
			if (!satisfied) {
				clauseIndices.add(i);
			}
