...
public class JFXFileViewer extends Application {
	...
	public void start(Stage primaryStage) {
		...
		fileButton.setOnAction(e -> {openFile(primaryStage);});	// to open the source where it roots
	...
	private void readCircles(File file) {
		try {
			...
			//if there is a problem, shoe an error message.
			e.printStackTrace();
			//AlertType.ERRO?
			...
			else {	// why press X it exits but not other keys? this is just a else
				System.exit(-1);
			}
		}
	}
