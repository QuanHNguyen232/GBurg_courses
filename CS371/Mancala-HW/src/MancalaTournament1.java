...
public class MancalaTournament1 {
	...
	public static void main(String[] args) throws Exception {
		...
		String[] playerIdentifiers = {"QuDo2", "QuDo1"};
		...
		for (int i = 0; i < competitors.length; i++) {
			System.out.println(i + "\t" + playerIdentifiers[i] + "\t" + wins[i] + "\t" + totalWins[i] + "\t" + points[i]);
		}
	}

	public static int[] runGames(String player1, String player2, PrintWriter p) {
		try {
			...
			int[] scores = new int[4]; // Player 1 games won, player 2 games won, player 1 points scored, player 2 points scores
			...
			for (int g = 0; g < NUM_GAMES; g++) { // best of NUM_GAMES
				...
				MancalaNode node = new ScoreDiffMancalaNode(9999);
				p.println(node);
				...
				while (!node.gameOver()) {
					...
					clock.reset();
					clock.start();
					//move = player[node.player].chooseMove((MancalaNode) node.clone(), playerMillisRemaining[node.player]);
					move = players[node.player].chooseMove(node, playerMillisRemaining[node.player]);
		...
		System.out.println("Player 1 moves: " + p1Moves);
		System.out.println("Player 2 moves: " + p2Moves);
		System.out.println("Player 1 avg score: " + (double) scores[2] / p1Moves);
		System.out.println("Player 2 avg score: " + (double) scores[3] / p2Moves);
		p.flush();
		return scores;
